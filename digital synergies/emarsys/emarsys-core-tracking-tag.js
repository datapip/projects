 ...
 *   {{CJS - Emarsys Category}}       CJS Var    — category label derived from page.name
 *
...
(function () {
  ...
  if ("{{Event}}" === "view_item") {
    ...
    var category; //= {{CJS - Emarsys Category}};
    if (category) {
  ...
  if ("{{DLV - page.type}}" === "product") {
    return;
  }
  if ("{{DLV - page.name}}" === "checkout:basket") {
    return;
  }
  if ("{{DLV - page.name}}" === "checkout:order") {
    return;
  }
    ...
    try {
      ...
      for (var i = dl.length - 1; i >= 0; i--) {
        ...
        if (o.user && o.user.email)
          return String(o.user.email).trim().toLowerCase();
      }
    } catch (e) {}
    try {
      var ls = localStorage.getItem("em_e");
      if (ls) return ls.trim().toLowerCase();
    } catch (e) {}
    try {
